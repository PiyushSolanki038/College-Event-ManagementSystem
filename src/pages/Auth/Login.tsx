...
import { Loader2, Mail, LockKeyhole, Eye, EyeOff, CheckCircle2, ChevronRight, X } from 'lucide-react';
  ...
  const handleSocialAuth = async () => {
    ...
    try {
      ...
      try {
        ...
        // If registration also fails, try login again (account might exist with different password)
        setSocialError(regErr?.message || `${provider === 'google' ? 'Google' : 'Apple'} sign-in failed. Try using email/password login instead.`);
      }
    } finally {
            ...
            >
              ...
              <div style={{ textAlign: 'center', marginBottom: 28 }}>
                ...
                <p style={{ fontSize: 13, color: '#94a3b8', marginTop: 8, fontWeight: 500 }}>
                  Enter your {socialModal === 'google' ? 'Google' : 'Apple ID'} email to continue as <strong style={{ color: '#f97316' }}>{role}</strong>
                </p>
              </div>
